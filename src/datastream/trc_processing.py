"""TRC file processing utilities.

Contains functions for smoothing and manipulating TRC marker data.
"""
...
def smooth_trc(
    ...
    print(f"[smooth] Temporal smoothing (window={window}) on {num_frames} frames, {num_markers} markers")
...
def hide_markers_in_trc(
    ...
    # Line 3 (index 3) contains marker names
    marker_line = lines[3]
    ...
    # Process data lines (starting at line 5)
    new_lines = lines[:5]
    for line in lines[5:]:
        if not line.strip():
