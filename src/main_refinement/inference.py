...
1. Loads all three models (POF, joint, main refiner)
...
Usage:
    ...
    pipeline = MainRefinerPipeline(
        pof_checkpoint='models/checkpoints/best_pof_model.pth',
...
class MainRefinerPipeline:
    ...
    This class handles:
    1. Loading POF, joint, and main refiner models
    ...
    def __init__(
        ...
        Args:
            pof_checkpoint: Path to POF model checkpoint
        ...
        # Load POF model
        self.pof_model = self._load_pof_model(pof_checkpoint)
        ...
        print(f"  POF model: {sum(p.numel() for p in self.pof_model.parameters()):,} params")
    ...
    def _load_pof_model(self, checkpoint_path: Union[str, Path]) -> torch.nn.Module:
        """Load POF model."""
        checkpoint_path = Path(checkpoint_path)
        if not checkpoint_path.exists():
            raise FileNotFoundError(f"POF model not found: {checkpoint_path}")

        model = load_pof_model(str(checkpoint_path), device=self.device, verbose=False)
        model.eval()
    ...
    def _refine_single(
        ...
        # 1. Run POF model to get limb orientations and reconstructed pose
        pof_outputs = self._run_pof_model(pose_2d, visibility)

        # 2. Run joint model (compute angles first)
        joint_outputs = self._run_joint_model(pose_normalized, visibility)

        # 3. Run main refiner
        main_output = self.main_model(pose_t, vis_t, pof_outputs, joint_outputs)
        ...
        return {
            ...
            'pof_weights': main_output['pof_weights'].squeeze(0).cpu().numpy(),
    ...
    def _run_pof_model(
        ...
        Args:
            pose_2d: (17, 2) 2D keypoints
            visibility: (17,) visibility scores

        Returns:
            Dict with 'pof_vectors' and 'reconstructed_pose' tensors
        """
        from src.pof.dataset import normalize_pose_2d, compute_limb_features_2d

        # Normalize 2D and compute limb features
        pose_2d_norm, _, _ = normalize_pose_2d(pose_2d)
        limb_delta_2d, limb_length_2d = compute_limb_features_2d(pose_2d_norm)

        # Convert to tensors
        pose_2d_t = torch.from_numpy(pose_2d_norm.astype(np.float32)).unsqueeze(0).to(self.device)
        vis_t = torch.from_numpy(visibility.astype(np.float32)).unsqueeze(0).to(self.device)
        limb_delta_t = torch.from_numpy(limb_delta_2d.astype(np.float32)).unsqueeze(0).to(self.device)
        limb_length_t = torch.from_numpy(limb_length_2d.astype(np.float32)).unsqueeze(0).to(self.device)

        # Run POF model
        with torch.no_grad():
            pof_vectors = self.pof_model(pose_2d_t, vis_t, limb_delta_t, limb_length_t)

        # Reconstruct 3D pose from POF (simplified - uses normalized space)
        # In full pipeline, POF reconstruction happens via least_squares solver
        # Here we just provide the POF vectors and a placeholder reconstructed pose
        from src.pof.reconstruction import reconstruct_skeleton_least_squares

        pof_np = pof_vectors.cpu().numpy()
        reconstructed = reconstruct_skeleton_least_squares(
            pof_np, pose_2d[np.newaxis, ...], None,
            pelvis_depth=0.0, denormalize=False
        )
        reconstructed_t = torch.from_numpy(reconstructed.astype(np.float32)).to(self.device)

        return {
            'pof_vectors': pof_vectors,
            'reconstructed_pose': reconstructed_t,
            'confidence': torch.ones(1, 17, device=self.device),  # Default confidence
        }
...
if __name__ == '__main__':
    ...
    print("Usage: MainRefinerPipeline(pof_checkpoint, joint_checkpoint, main_checkpoint)")
