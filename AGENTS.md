...
Instructions for AI assistants (Claude Code, ChatGPT Codex, and other AI coding tools) working on this codebase.
...
| `application/config/` | Path resolution (`UserPaths`, `AppPaths`), model downloads |
...
**Location**: `~/.humanpose3d/data/output/pose-3d/<video>/`

```
~/.humanpose3d/data/output/pose-3d/<video>/
...
**Note**: All data now stored in home directory (`~/.humanpose3d`) for portability.
...
7. **Path management is centralized**: All paths resolved through `UserPaths` (`~/.humanpose3d`), never hardcode paths
...
- Path management: [docs/PATHS.md](docs/PATHS.md)
- Migration guide: [docs/MIGRATION_TO_HOME_DIR.md](docs/MIGRATION_TO_HOME_DIR.md)
